...
char*  msstrtok_r(char* string_org,const char* demial)
{
...
char*  bsdstrtok_r(char* string_org,const char* demial,char** last)
{
...
int main()
{
...
#else
    while((p[in]=msstrtok_r(buf,","))!=NULL) {
        buf=p[in];
        while((p[in]=msstrtok_r(buf," "))!=NULL) {
            in++;
            buf=NULL;
    ...
    char *outer_ptr=NULL;
    char *inner_ptr=NULL;
    while((p[in]=bsdstrtok_r(buf,",",&outer_ptr))!=NULL) {
        buf=p[in];
        while((p[in]=bsdstrtok_r(buf," ",&inner_ptr))!=NULL) {
            in++;
            buf=NULL;
        }
        p[in++]="***";
